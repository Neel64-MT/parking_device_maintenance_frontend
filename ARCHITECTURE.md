...
| Photo picker files | Local `File[]` + object-URL thumbs in PhotoPicker (`max` 5); parent holds files until submit |
...
- **Phase 27 — QR → device → raise/update:** `resolveScan` → `GET /api/devices/scan?q=` (404 → null; other errors rethrown). One lookup returns device + `openTicketId` (no `/tickets/by-slot`). Raise: free device → issue UUIDs from `GET /api/issues` → `uploadImages` → `POST /api/tickets`; `409 OPEN_TICKET_EXISTS` / `REOPEN_SAME_TICKET` → existing ticket. Open ticket → Open / Update existing ticket → `/tickets/:id` (Detail Add Update). Scan QR same branching; no simulate-mock buttons.
- **Phase 27b — Update Ticket live scan:** `/tickets/update` uses the same `resolveScan`; open ticket → Detail; no open ticket → Raise CTA; miss/error empty states; mock TK-1042 form removed.
...
TicketDetail
  ...
  → Add Update → Modal → POST updates → uploads → PATCH photos; work history ASC
...
DeviceList (canPerm Device list c)
  ...
  → completed → toast (Created/Updated/Skipped from stats when present) + reloadToken → GET /api/devices (same page/filters)
